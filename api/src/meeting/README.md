...
2. **Callback** – ai-engine-2 `POST`s to Nest:
   ...
   Body: `WorkerResultPayload` (see `dto/worker-result.dto.ts`). Payload includes `new_tasks`, `transitioned_tasks`, `blockers`, `summary`, `insights`, and `suggested_actions`. Blockers are persisted to **TranscriptBlocker**.
...
**Project member directory for assignee IDs:** `GET /internal/projects/members?project_id=<uuid>` → `{ members: [{ id, name, email, role, aliases[] }] }`. Use this to map diarized speakers/LLM assignees to canonical `assigneeId` before callback.
